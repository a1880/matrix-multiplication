...
class BiniScheme(object):
    ...
    def __init__(self, transpose_matrix_c: bool = False):
        ...
        self.alpha: list[list[list[int]]] = []
        """ 3D array  [row][col][product] """

        self.beta: list[list[list[int]]] = []
        """ 3D array  [row][col][product] """

        self.gamma: list[list[list[int]]] = []  # 3D array
        """ 3D array  [row][col][product] """
    ...
    @staticmethod
    def array_cols(arr: list) -> int:
        return len(arr[0])

    @staticmethod
    def array_rows(arr: list) -> int:
        return len(arr)

    def a(self, row: int, col: int, product: int) -> int:
        return self.alpha[row][col][product]
    ...
    def auto_file_name(self) -> str:
        """ Return default file name for this Bini scheme"""
        return f"s{self.signature}.bini.txt"

    def b(self, row: int, col: int, product: int) -> int:
        return self.beta[row][col][product]
    ...
    def c(self, row: int, col: int, product: int) -> int:
        return self.gamma[row][col][product]
    ...
    def combine(self, bsx: 'BiniScheme', bsy: 'BiniScheme') -> None:
    ...
    def copy(self) -> 'BiniScheme':
        """ provide a deep copy of self """
        c = BiniScheme(self.transpose_matrix_c)
        c.alpha = self.copy_array(self.alpha)
        c.beta = self.copy_array(self.beta)
        c.gamma = self.copy_array(self.gamma)
        c.mod2_mode = self.mod2_mode
        c.output_file = None
        c.output_line_count = 0
        return c
    
    @staticmethod
    def copy_array(a):
        if hasattr(a, "__len__"):
            return [BiniScheme.copy_array(x) for x in a]
        return a
    
    def create_matrices(self, a_rows: int, a_cols: int, b_cols: int, 
                        no_of_products: int) -> None:
    ...
    @staticmethod
    def determine_matrix_order(line: str, matrix_order: list) -> list[int]:
    ...
    def fill_matrices(self, line: str, matrix_order: list) -> None:
    ...
    @staticmethod
    def literal_array(rows: int, cols: int, products: int) -> list[list[list[int]]]:
    ...
    def set_combined_matrix(self, m: list, 
                            x: list, x_products: range, 
                            y: list, y_products: range) -> None:
    ...
    def validate(self) -> None:
        ...
        if self.no_of_products > 343:
            print(f"Skipped/aborted validation due to high number of {self.no_of_products} products")
            return True
                    ...
                    for b_col in self.r_b_cols:
                        ...
                        for c_row in self.r_c_rows:
                            for c_col in self.r_c_cols:
                                ...
                                if self.mod2_mode:
                                    eq_sum = eq_sum % 2
                                if (odd and (eq_sum != 1)) or (not odd and (eq_sum != 0)):
                                    errors += 1
                                    if errors == 1:
                                        o()
                                        o(f"Bummer! Error(s) found. eq {equations} " +
                                          f"odd {odd} is: {eq_sum} expected: {1 if odd else 0}")
                                        o()
