...
def show_elapsed(started, purpose):
    """
    helper to display the elapsed time
    """
    print()
    elapsed = time.time() - started
    msg = f"Execution time for {purpose}: "
    if elapsed < 1:
        print(f"{msg}{int(elapsed * 1000)} ms")
    else:
        print(f"{msg}{round(elapsed, 1)} s")
...
class akSmtSolver:
    ...
    def __init__(self, solver, variables, smt_solver):
        """
    ...
    def check(self):
        ...
        show_elapsed(started, self.smt_solver)
...
class akSmtSolver_model:
    ...
    def read_solution(self, file_name):
        ...
        print(f"Solution file read complete. Lines: {true_line_count}")
